...
function repairCase(attackType: 'patch' | 'fgsm'): ActiveStage {
  ...
  const playerCase: PlayerCase = {
    ...
    parameter_rules: attackType === 'patch' ? [
      ...
      { parameter: 'epsilon_pixels', allowed_values: [0.25, 1, 4], initial_value: 4, fallback_value: 0.25 },
    ],
    max_attempts: 3,
    available_states: [],
  }
...
describe('RepairInvestigationFlow', () => {
  ...
  test('shows the initial error and requires a repair reason before manipulation', async () => {
    ...
    expect(screen.getAllByRole('radio', { name: /Patch|size/i })).toHaveLength(4)
    expect(screen.getByRole('radio', { name: 'Remove the Patch' })).toBeInTheDocument()
    await user.click(screen.getByRole('radio', { name: 'Reduce the Patch size' }))
    await user.click(screen.getByRole('radio', { name: 'The correct classification will be restored' }))
    const continueButton = screen.getByRole('button', { name: 'Continue to Manipulate' })
    expect(continueButton).toBeDisabled()
    await user.type(screen.getByPlaceholderText(/Explain your repair idea/i), 'A smaller patch may reveal more of the traffic light.')
    ...
    expect(screen.getByText('A smaller patch may reveal more of the traffic light.')).toBeInTheDocument()
    ...
    expect(JSON.parse(String(reclassifyCall?.[1]?.body))).toMatchObject({
      predicted_outcome: 'restored',
      prediction_reason: 'A smaller patch may reveal more of the traffic light.',
