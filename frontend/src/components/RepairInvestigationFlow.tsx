...
export function RepairInvestigationFlow({ cases, nextError, onStageComplete, onContinue, isMovingNext }: RepairInvestigationFlowProps) {
  ...
  const latestMethodAttempt = methodAttempts.filter((attempt) => !attempt.fallback).at(-1)
  const pixelCurrentImageUrl = resolveApiUrl(latestMethodAttempt?.action.image_url ?? selectedCase.playerCase.initial_image_url)
  const pixelCurrentStrength = Number(latestMethodAttempt?.parameters.epsilon_pixels ?? initialParameters(selectedCase).epsilon_pixels ?? 0)
  const pixelPreviewReady = selectedMethod === 'Pixel' && !parametersUnchanged && previewUrl !== null && !isPreviewing

  function renderPreviousAttemptRecords(method: Method) {
    const previousAttempts = attempts.filter((attempt) => attempt.method === method && !attempt.fallback)
    return <div className="stage-three-attempt-records">{previousAttempts.length ? <ol>{previousAttempts.map((attempt, index) => <li key={`${attempt.action.attempt_number}-${index}`}><strong>Attempt {index + 1}: {parameterSummary(attempt.parameters)}</strong><span>{attempt.action.classification_restored ? `Restored ${selectedCase.playerCase.correct_label}` : `Still misclassified as ${attempt.action.top1.label}`}</span></li>)}</ol> : <p>No previous {method} attempts yet.</p>}<p className="stage-three-evidence-next"><strong>Evidence → next step</strong><span>Use the evidence from your previous attempts to decide what to try next.</span></p></div>
  }
  ...
  async function finishReflection() {
    ...
    try {
      ...
      await saveStageResponse(primaryRun, { question_key: 'stage3_investigation_order', question_version: 4, answer_type: 'multiple_choice', answer_json: orderResult })
  ...
  function renderStageThreePixelManipulate() {
    return <PixelManipulateLayout modules={[
      {
        key: 'stage-three-whole-images',
        title: 'Before · Current Result | Preview · Selected Strength',
        description: 'Choose a new Strength to generate a separate preview from the fixed image baseline.',
        content: <div className="stage-two-pixel-whole-images">
          <FixedPixelRegionImage title="Before · Current result" imageUrl={pixelCurrentImageUrl} alt="Traffic light current Pixel result with fixed 32 by 32 region" details={`Current Strength: ${pixelCurrentStrength}/255`} />
          {pixelPreviewReady ? <FixedPixelRegionImage title="Preview · Selected Strength" imageUrl={previewUrl!} alt="Traffic light preview at selected Pixel Strength with fixed 32 by 32 region" details={`Selected Strength: ${currentParameters.epsilon_pixels}/255 · preview only`} /> : <figure className="image-preview-card stage-two-pixel-preview-placeholder"><div className="card-heading"><strong>Preview · Selected Strength</strong></div><div role="status">Choose a new Pixel Strength to preview the modified image.</div></figure>}
        </div>,
      },
      {
        key: 'stage-three-strength',
        title: 'Select a Pixel Strength',
        description: <>Current result Strength: <strong>{pixelCurrentStrength}/255</strong>. Select a different, untested value for this attempt.</>,
        content: <><p className="stage-three-repair-goal"><strong>Repair Goal</strong><span>Find a setting that restores the correct classification.</span></p><PixelStrengthControl values={selectedCase.playerCase.parameter_rules[0].allowed_values ?? []} value={currentParameters.epsilon_pixels} currentValue={parametersBeforeCurrentRepair.epsilon_pixels ?? 0} initialValue={Number(initialParameters(selectedCase).epsilon_pixels ?? 0)} learningMode="compact" baselineUnlocked={pixelBaselineUnlocked} onChange={(value) => updateParameter('epsilon_pixels', value)} /><p className="fixed-baseline-note"><strong>How this preview is generated:</strong> Each Strength creates a separate candidate from the same fixed image baseline. Strengths are not added to previous attempts.</p></>,
      },
      {
        key: 'stage-three-region',
        title: '32×32 Selected Region',
        description: 'The same fixed region provides a basic visual preview of the current result and selected Strength candidate.',
        content: <PixelInspector mode="preview" originalUrl={pixelCurrentImageUrl} modifiedUrl={pixelPreviewReady ? previewUrl! : pixelCurrentImageUrl} subject="traffic light" observedStrength={pixelCurrentStrength} strength={currentParameters.epsilon_pixels ?? pixelCurrentStrength} previewReady={pixelPreviewReady} beforeLabelOverride="Current result" afterLabelOverride="Preview" />,
      },
      {
        key: 'stage-three-attempts',
        title: 'Previous attempts / results',
        content: renderPreviousAttemptRecords('Pixel'),
      },
    ]} />
  }
  ...
  return <div className="repair-flow">
    ...
    <SimulationNotice>Stage 3 lets you try different repair strategies and check the classification result. You have up to three autonomous attempts per attack.</SimulationNotice>
    ...
    {phase === 'manipulate' ? <>
      <PanelTitle label="Manipulate" title={directionOptions(selectedMethod).find(([value]) => value === direction)?.[1] ?? `Adjust the ${selectedMethod}`} description={selectedMethod === 'Pixel' ? `Repair goal: Find a setting that restores the correct classification. ${attemptsRemaining} attempts remaining.` : `Attempts remaining before this repair: ${attemptsRemaining}. Adjust the parameters, then continue without seeing the result.`} />
      {selectedMethod === 'Pixel' ? renderStageThreePixelManipulate() : <><section className="single-parameter-experiment"><div className={`experiment-preview ${isPreviewing ? 'is-loading' : ''}`}><ImagePreviewCard title={isPreviewing ? 'Updating repair preview…' : 'Repair preview'} imageUrl={previewUrl ?? resolveApiUrl(selectedCase.playerCase.initial_image_url)} alt="Traffic light Patch repair preview" details={<span>{parameterSummary(currentParameters)}</span>} /></div><div className="experiment-controls"><div className="card-heading"><strong>Patch repair settings</strong><StatusBadge tone="neutral">{attemptsRemaining} attempts left</StatusBadge></div>{direction !== 'reduce_patch' ? ['position_x', 'position_y'].map((name) => { const rule = selectedCase.playerCase.parameter_rules.find((item) => item.parameter === name); const values = rule?.allowed_values ?? []; const index = values.indexOf(currentParameters[name]); return <label className="repair-slider" key={name}><span>{name === 'position_x' ? 'Horizontal position' : 'Vertical position'} <strong>{currentParameters[name]?.toFixed(2)}</strong></span><input type="range" min="0" max={Math.max(0, values.length - 1)} value={Math.max(0, index)} onChange={(event) => updateParameter(name, values[Number(event.target.value)])} /></label> }) : <p className="fixed-parameter-note">Current Patch position: X {currentParameters.position_x.toFixed(2)}, Y {currentParameters.position_y.toFixed(2)}. Position stays fixed for this repair direction.</p>}{direction !== 'move_patch' ? <fieldset className="repair-size-options"><legend>Patch size</legend><div className="patch-option-row">{nonZeroValues(selectedCase.playerCase.parameter_rules.find((item) => item.parameter === 'size_fraction')?.allowed_values).map((value) => <button key={value} type="button" className={currentParameters.size_fraction === value ? 'is-selected' : ''} aria-pressed={currentParameters.size_fraction === value} onClick={() => updateParameter('size_fraction', value)}>{Math.round(value * 100)}%</button>)}</div></fieldset> : <p className="fixed-parameter-note">Current Patch size: {Math.round(currentParameters.size_fraction * 100)}%. Size stays fixed for this repair direction.</p>}<div className="repair-plan-reminder"><strong>Your repair direction</strong><span>{directionOptions(selectedMethod).find(([value]) => value === direction)?.[1]}</span></div></div></section><section className="stage-three-previous-attempts" aria-label="Previous Patch attempts and results"><h3>Previous attempts / results</h3>{renderPreviousAttemptRecords('Patch')}</section></>}
    ...
    {phase === 'reclassify' ? <>
      <PanelTitle label="Reclassify" title={currentAttempt ? 'Repair result revealed' : 'Test this repair with the classifier'} description="Send the repaired image to the classifier, then reveal and compare its classification result." />
    ...
    {phase === 'compare' && currentAttempt ? <>
      ...
      {selectedMethod === 'Pixel' ? <>
        <PixelCompareLayout mode="three-state" attackedImageUrl={resolveApiUrl(selectedCase.playerCase.initial_image_url)} attackedStrength={Number(initialParameters(selectedCase).epsilon_pixels ?? 0)} attackedPrediction={selectedCase.playerCase.initial_top1} adjustedImageUrl={resolveApiUrl(currentAttempt.action.image_url)} adjustedStrength={Number(currentAttempt.parameters.epsilon_pixels ?? 0)} adjustedPrediction={currentAttempt.action.top1} correctLabel={selectedCase.playerCase.correct_label} subject="traffic light" classificationRestored={currentAttempt.action.classification_restored} revealOriginalStrength={currentAttempt.action.classification_restored || currentAttempt.fallback || currentAttempt.action.attempts_remaining === 0} showConfidenceExplanation={false} />
      ...
      {currentAttempt.fallback ? <section className="verified-fallback"><p className="step-label">One verified repair</p><h3>Available repair parameters</h3><p><strong>After three unsuccessful repair attempts, the system applied a repair that had been verified in advance for this case.</strong></p><strong>{parameterSummary(currentAttempt.parameters)}</strong></section> : null}
    ...
    {phase === 'fallback' && currentAttempt?.fallback ? <>
      ...
      <section className="verified-fallback"><p className="step-label">One verified repair</p><h3>Available repair parameters</h3><strong>{parameterSummary(currentAttempt.parameters)}</strong><p>This is one verified solution for this case, shown after the three autonomous attempts.</p></section>
    ...
    {phase === 'reflect-order' ? <>
      <PanelTitle label="Reflection" title="Rebuild the investigation process" description="Arrange the six steps without relying on a fixed Patch or Pixel setting." />
      <InvestigationOrderTask onComplete={(result) => setOrderResult(result)} />
      <div className="stage-navigation stage-navigation--end"><button className="primary-button" type="button" disabled={!orderResult} onClick={() => setPhase('reflect-concept')}>Next question</button></div>
    </> : null}
    ...
    {phase === 'summary' ? <div className="standalone-summary"><Sparkles size={30} /><p className="step-label">Repair Investigation summary</p><h2>A process you can reuse</h2><p><strong>Patch and Pixel were examples used for practice. The reusable learning outcome is the investigation process.</strong></p><p>Image changes can take many forms, and the same settings may produce different results on different images.</p><section className="stage-three-process-summary" aria-label="Reusable investigation process"><strong>Reusable investigation process</strong><p>Observe the image and current classification, predict a direction to test, change one factor, reclassify, compare the evidence, and decide what to investigate next.</p></section><p>Use the evidence from each result to decide what to test next. Do not assume that a setting that worked once will work for every image.</p><button className="primary-button" type="button" onClick={() => setPhase('complete')}>Continue</button></div> : null}
