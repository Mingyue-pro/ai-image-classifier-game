...
const INITIAL_PARAMETERS: ComplexTransferParameters = {
  patch: { size: 0.30, positionX: 0.60, positionY: 0.20 },
  pixelStrength: 4,
  blurLevel: 'high',
}

const BLUR_RADII = { high: 16, medium: 8, low: 4, none: 0 } as const
const PIXEL_STRENGTHS = [
  ['High', 4],
  ['Medium', 2],
  ['Low', 1],
  ['Remove', 0],
] as const
const PATCH_SIZES = [0, 0.10, 0.20, 0.30] as const
...
export function ComplexTransferFlow({ imageUrl, subject, currentPrediction, attemptIndex = 0, maxAttempts = 5, initialClassification = currentPrediction.label, attemptHistory = [], runFinished = false, runSuccess = false, referenceParameters, referenceTop1Label = subject, onPlanConfirmed, currentParameters = INITIAL_PARAMETERS, onManipulationConfirmed, onPreview, onReclassify, onDecideNext, reflectionCompleted = false, onSubmitReflection, onViewReport, onTimingEvent }: ComplexTransferFlowProps) {
  ...
  function attemptParameterChange(attempt: ComplexTransferAttempt): string {
    ...
    return `${attempt.before_parameters.blur_level} · radius ${BLUR_RADII[attempt.before_parameters.blur_level]} → ${attempt.after_parameters.blur_level} · radius ${BLUR_RADII[attempt.after_parameters.blur_level]}`
  }
  ...
  return <div className="complex-transfer-flow">
    ...
    {phase === 'manipulate' ? <>
      ...
      {activeFactor ? <div className={activeFactor === 'pixel' ? 'single-parameter-experiment' : 'complex-transfer-manipulate-layout'}>
        {activeFactor === 'pixel' ? <div className="experiment-preview"><PixelInspector mode="comparison" originalUrl={imageUrl} modifiedUrl={previewUrl ?? imageUrl} subject={subject} observedStrength={currentParameters.pixelStrength} strength={draftParameters.pixelStrength} overviewState="original" /></div> : <ImagePreviewCard title={previewUrl ? 'Preview of this change' : 'Current image before this change'} imageUrl={previewUrl ?? imageUrl} alt={`Current Complex Transfer image showing ${subject}`} />}
        <section className={activeFactor === 'pixel' ? 'experiment-controls complex-transfer-tool' : 'complex-transfer-tool'} aria-label={`${activeFactor} manipulation tool`}>
          <div className="complex-transfer-carried-state"><strong>Current state carried into this attempt</strong><span>Patch: {patchSummary(currentParameters)}</span><span>Pixel: {currentParameters.pixelStrength}/255</span><span>Blur: {currentParameters.blurLevel} · radius {BLUR_RADII[currentParameters.blurLevel]}</span></div>
          {activeFactor === 'patch' ? <>
          ...
          {activeFactor === 'pixel' ? <fieldset><legend>Pixel-level modification strength</legend><div className="complex-transfer-level-options">{PIXEL_STRENGTHS.map(([label, value]) => <button type="button" key={label} className={draftParameters.pixelStrength === value ? 'is-selected' : ''} aria-pressed={draftParameters.pixelStrength === value} onClick={() => setDraftParameters((current) => ({ ...current, pixelStrength: value }))}><strong>{label}</strong><span>{value}/255</span></button>)}</div><p className="factor-neutral-note">Strength controls how far pixel values may shift from the original. Test the selected value with the classifier; no value is presented as universally best.</p></fieldset> : null}
          {activeFactor === 'blur' ? <fieldset><legend>Blur / image clarity</legend><div className="complex-transfer-level-options">{(['high', 'medium', 'low', 'none'] as const).map((level) => <button type="button" key={level} className={draftParameters.blurLevel === level ? 'is-selected' : ''} aria-pressed={draftParameters.blurLevel === level} onClick={() => setDraftParameters((current) => ({ ...current, blurLevel: level }))}><strong>{level}</strong><span>Radius {BLUR_RADII[level]}</span></button>)}</div></fieldset> : null}
        </section>
      </div> : null}
    ...
    {phase === 'decide' && reclassifyResult ? <>
      ...
      <section className="complex-transfer-decide-next" aria-label="Next factor selection">
        <p><strong>What would you like to investigate next?</strong> You may continue with the same factor or investigate a different one.</p>
    ...
    {phase === 'summary' ? <>
      ...
      {summarySuccess ? <section className="complex-transfer-final-state" aria-labelledby="complex-transfer-final-repair-title"><h3 id="complex-transfer-final-repair-title">Final repair parameters</h3><dl><div><dt>Patch</dt><dd>{patchStateSummary(summaryFinalParameters)}</dd></div><div><dt>Pixel strength</dt><dd>{summaryFinalParameters.pixelStrength}/255</dd></div><div><dt>Blur</dt><dd>{summaryFinalParameters.blurLevel} · radius {BLUR_RADII[summaryFinalParameters.blurLevel]}</dd></div><div><dt>Final classification</dt><dd>{summaryFinalClassification}</dd></div></dl></section> : null}
      ...
      {summarySuccess ? <p className="complex-transfer-neutral-summary">These changes were associated with the classification returning in this case. The results show what happened for this image and this model under the changes you tested.</p> : <>
        ...
        {referenceParameters ? <>
          <section className="complex-transfer-state-comparison" aria-labelledby="complex-transfer-state-comparison-title"><h3 id="complex-transfer-state-comparison-title">Your final state vs one verified reference</h3><p>One verified reference is a parameter configuration that produced the correct classification for this case. Other parameter combinations may also produce different results.</p><div><section aria-label="Your final state"><h4>Your final state</h4><dl><div><dt>Patch</dt><dd>{patchStateSummary(summaryFinalParameters)}</dd></div><div><dt>Pixel strength</dt><dd>{summaryFinalParameters.pixelStrength}/255</dd></div><div><dt>Blur</dt><dd>{summaryFinalParameters.blurLevel} · radius {BLUR_RADII[summaryFinalParameters.blurLevel]}</dd></div><div><dt>Final state classification</dt><dd>{summaryFinalClassification}</dd></div></dl></section><section aria-label="One verified reference"><h4>One verified reference</h4><dl><div><dt>Patch</dt><dd>{patchStateSummary(referenceParameters)}</dd></div><div><dt>Pixel strength</dt><dd>{referenceParameters.pixelStrength}/255</dd></div><div><dt>Blur</dt><dd>{referenceParameters.blurLevel} · radius {BLUR_RADII[referenceParameters.blurLevel]}</dd></div><div><dt>Verified classification</dt><dd>{referenceTop1Label}</dd></div></dl></section></div></section>
        </> : null}
      </>}
    ...
    {phase === 'complete' ? <>
      <PanelTitle label="Complex Transfer" title="Transfer complete" description="Your reflection has been saved." />
