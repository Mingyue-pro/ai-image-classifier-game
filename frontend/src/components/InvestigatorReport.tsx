...
function complexParameterRows(values: Record<string, unknown> | null) {
  ...
  return [
    ...
    { label: 'Blur Strength', value: `${String(values.blur_level ?? 'Not recorded')} · Setting ${Number(values.blur_radius ?? 0)}` },
  ]
}
...
export function InvestigatorReport({ sessionId, onHome }: Props) {
  ...
  const stageEvidenceCount = stageTables.reduce((total, table) => total + table.rows.length, 0)
  return <section className="report-page" aria-labelledby="report-title">
    ...
    <section className="report-card report-metrics" aria-label="Learning overview">
      ...
      <div><MetricLabel help="The number of scored predictions that matched the observed result. ‘Not sure’ answers and system-provided verified fallback records are excluded from both the matched count and the scored-prediction total. They remain included in Classification checks.">Predictions matching results</MetricLabel><strong>{overview.decisive_predictions ? `${overview.prediction_matches}/${overview.decisive_predictions}` : 'No decisive predictions'}</strong></div>
