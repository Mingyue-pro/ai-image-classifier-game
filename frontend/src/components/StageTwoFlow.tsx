...
import { PixelLearningNote, PixelStrengthChange } from './PixelLearning'
...
export function StageTwoFlow({ cases, nextError, onStageComplete, onContinue, isMovingNext }: StageTwoFlowProps) {
  ...
  const latestPixelStrength = latestAttempt('Pixel')?.parameters.epsilon_pixels
    ?? initialParameters(cases.find((item) => methodFor(item) === 'Pixel')?.playerCase.parameter_rules ?? []).epsilon_pixels
    ?? 0
