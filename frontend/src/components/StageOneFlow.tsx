...
import type { ActiveStage, GameAction, PlayerCaseState, StageOneSummary } from '../types'
...
function describeParameterValues(parameters: Record<string, unknown>): string {
  if ('epsilon_pixels' in parameters) return `Strength ${String(parameters.epsilon_pixels)}/255`
  const details: string[] = []
  if (typeof parameters.size_fraction === 'number') details.push(`Size ${Math.round(parameters.size_fraction * 100)}%`)
  if (typeof parameters.position_x === 'number' && typeof parameters.position_y === 'number') {
    details.push(`Position ${parameters.position_x}, ${parameters.position_y}`)
  } else details.push('Centre position')
  return details.join(' · ')
}

function describeParameters(state: PlayerCaseState): string {
  return describeParameterValues(state.parameters)
}
...
export function StageOneFlow({ cases, isMovingNext, nextError, priorSummaries, onStageComplete, onContinue }: StageOneFlowProps) {
  ...
  return (
    <div className="stage-one-flow">
      ...
      <SimulationNotice />
      {phase !== 'observe' ? <p className="test-counter"><strong>{selectedState ? `${methodName} investigation` : 'Stage 1 investigations'}</strong><span>{results.length} of 4 complete</span></p> : null}

      {phase === 'observe' ? <>
        <PanelTitle label="Observe" title="Start with the unmodified image" description="Inspect the image and its verified baseline classification before choosing a modification." />
        ...
        <div className="stage-navigation stage-navigation--end"><button className="primary-button" type="button" onClick={() => setPhase('select')}>Next: select a change</button></div>
      </> : null}

      {phase === 'select' ? <>
        <PanelTitle label="Observe · Select change" title="Choose one of four fixed investigations" description="All four Patch and Pixel scenarios are available now. Choose any unfinished scenario." />
        <div className="preset-grid">
          {scenarios.map(({ activeCase, state }) => {
            ...
            return <button key={`${activeCase.playerCase.case_id}-${state.state_id}`} type="button" className={`preset-card ${used ? 'is-complete' : ''}`} disabled={used} onClick={() => choosePreset(state.state_id)}><span className="scenario-icon">{scenarioMethod === 'Patch' ? <ScanSearch size={18} /> : <Sparkles size={18} />}</span><strong>{scenarioMethod} scenario</strong><span>{describeParameters(state)}</span>{used ? <em>Completed</em> : <small>View plan and predict</small>}</button>
          })}
        </div>
      ...
      {phase === 'predict' && selectedState ? <>
        ...
        <div className="modification-summary"><strong>{methodName} plan</strong><span>{describeParameters(selectedState)}</span></div>
        <fieldset className="choice-group"><legend>What will happen to Top-1?</legend>{[
          ['classification_changes', 'Top-1 will change'], ['classification_stays_same', 'Top-1 will not change'], ['uncertain', 'Not sure'],
        ...
        <div className="stage-navigation"><button className="secondary-button" type="button" onClick={() => setPhase('select')}>Back to Select</button><button className="primary-button" type="button" disabled={!prediction} onClick={() => setPhase('manipulate')}>Lock prediction and continue</button></div>
      </> : null}

      {phase === 'manipulate' && selectedState ? <>
        <PanelTitle label="Manipulate" title="Apply the fixed modification" description={methodName === 'Pixel' ? 'Select Apply, then click the image in Pixel Inspector to compare the same 32×32 region.' : 'Select Apply to view the fixed Patch. The classification remains hidden.'} />
        {methodName === 'Pixel' && applied ? <PixelInspector originalUrl={resolveApiUrl(playerCase.initial_image_url)} modifiedUrl={resolveApiUrl(selectedState.image_url)} subject={playerCase.subject} strength={Number(selectedState.parameters.epsilon_pixels ?? 0)} /> : <div className="manipulation-preview"><img src={resolveApiUrl(applied ? selectedState.image_url : playerCase.initial_image_url)} alt={applied ? `Modified ${playerCase.subject} preview` : `Original ${playerCase.subject}`} /></div>}
      ...
      {phase === 'reclassify' && selectedState ? <>
        <PanelTitle label="Reclassify" title={result ? 'New classification result' : 'Request the verified result'} description={result ? 'Review the newly revealed result, then continue to the full comparison.' : 'Your prediction is now locked. Reclassify to reveal the offline-verified ResNet-34 result.'} />
        <div className="reclassify-action-layout"><div>{methodName === 'Pixel' ? <PixelInspector originalUrl={resolveApiUrl(playerCase.initial_image_url)} modifiedUrl={resolveApiUrl(selectedState.image_url)} subject={playerCase.subject} strength={Number(selectedState.parameters.epsilon_pixels ?? 0)} /> : <ImagePreviewCard title="Before reclassification" imageUrl={resolveApiUrl(selectedState.image_url)} alt="Modified image ready for reclassification" details={<span>{describeParameters(selectedState)}</span>} />}</div><div className="reclassify-center-action"><span>Send modified image to classifier</span><button className="primary-button" type="button" disabled={isSubmitting || result !== null} onClick={() => void reclassify()}>{isSubmitting ? 'Reclassifying…' : result ? 'Reclassified' : 'Reclassify image'}</button></div><ClassificationResultCard title="After reclassification" prediction={result?.top1} correctLabel={playerCase.correct_label} reveal={result !== null} /></div>
      ...
      {phase === 'compare' && result ? <>
        ...
        {methodName === 'Pixel' ? <><PixelInspector originalUrl={resolveApiUrl(playerCase.initial_image_url)} modifiedUrl={resolveApiUrl(result.image_url)} subject={playerCase.subject} strength={Number(selectedState?.parameters.epsilon_pixels ?? 0)} /><div className="comparison-evidence comparison-evidence--classification-only"><ClassificationResultCard title="Before classification" prediction={playerCase.initial_top1} correctLabel={playerCase.correct_label} /><ClassificationResultCard title="After classification" prediction={result.top1} correctLabel={playerCase.correct_label} /></div></> : <div className="comparison-evidence"><div><ImagePreviewCard title="Before" imageUrl={resolveApiUrl(playerCase.initial_image_url)} alt={`Original ${playerCase.subject}`} /><ClassificationResultCard title="Before classification" prediction={playerCase.initial_top1} correctLabel={playerCase.correct_label} /></div><div><ImagePreviewCard title="After" imageUrl={resolveApiUrl(result.image_url)} alt={`Modified ${playerCase.subject}`} /><ClassificationResultCard title="After classification" prediction={result.top1} correctLabel={playerCase.correct_label} /></div></div>}
        <dl className="comparison-facts"><div><dt>Modification</dt><dd>{selectedState ? describeParameters(selectedState) : ''}</dd></div><div><dt>Your prediction</dt><dd>{prediction.replaceAll('_', ' ')}</dd></div><div><dt>Actual result</dt><dd>{result.classification_changed ? 'Top-1 changed' : 'Top-1 did not change'}</dd></div></dl>
      ...
      {phase === 'reflection' ? <>
        ...
        <div className="stage-summary"><h3>Stage 1 evidence summary</h3><div className="summary-table" role="table" aria-label="Stage 1 results"><div className="summary-row summary-head" role="row"><span>Method</span><span>Parameters</span><span>Before → After</span><span>Prediction</span><span>Changed?</span><span>Matched?</span></div>{[...priorSummaries, ...methodSummaries].map((item, index) => <div className="summary-row" role="row" key={`${item.method}-${index}`}><span>{item.method}</span><span>{describeParameterValues(item.parameters)}</span><span>{item.top1_before} → {item.top1_after}</span><span>{item.prediction.replaceAll('_', ' ')}</span><span>{item.classification_changed ? 'Yes' : 'No'}</span><span>{item.prediction_matched === null ? 'Not sure' : item.prediction_matched ? 'Yes' : 'No'}</span></div>)}</div>{reflectionChoice ? <p className={`prediction-feedback ${reflectionChoice === 'depends_on_conditions' ? 'prediction-feedback--match' : ''}`}>{reflectionChoice === 'depends_on_conditions' ? 'Correct. Modification alone does not guarantee a changed classification.' : 'Review the evidence: some results changed and others did not.'}</p> : null}</div>
        <div className="stage-navigation stage-navigation--end"><button className="primary-button" type="button" disabled={!reflectionChoice || isSubmitting || isMovingNext} onClick={() => void finishCase()}>{isSubmitting || isMovingNext ? 'Saving…' : 'Complete Stage 1'}</button></div>
      </> : null}
