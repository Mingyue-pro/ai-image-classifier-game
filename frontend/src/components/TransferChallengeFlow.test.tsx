...
function transferCase(attackType: 'patch' | 'fgsm'): ActiveStage {
  ...
  return {
    ...
    playerCase: {
      ...
      ] : [{ parameter: 'epsilon_pixels', allowed_values: [0, 4], initial_value: 4, fallback_value: 0 }],
...
describe('TransferChallengeFlow', () => {
  ...
  test('saves the strategy feedback and one repair direction before manipulation', async () => {
    ...
    await user.click(screen.getByRole('radio', { name: /Make a specific prediction/ }))
    ...
    expect(screen.queryByRole('radio', { name: 'Reduce the Pixel attack strength' })).not.toBeInTheDocument()
    ...
    await user.type(screen.getByRole('textbox', { name: 'Why did you choose this repair direction?' }), 'Moving the Patch changes one controlled property.')
    expect(submit).toBeEnabled()
    await user.click(submit)

    expect(await screen.findByRole('heading', { name: 'Test your predicted repair direction' })).toBeInTheDocument()
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[3][1]?.body))).toMatchObject({ question_key: 'transfer_patch_repair_reason' })
  ...
  test('keeps the Patch image through its sub-case before moving to the Pixel image', async () => {
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      const body = url.endsWith('/preview')
        ...
        : url.endsWith('/reclassify')
          ? { attempt_number: 1, image_url: '/patch-result.png', top1: { label: 'ice cream', probability: 0.9, class_index: 2 }, top5: [], parameters: {}, classification_changed: true, correct_label_is_top1: true, classification_restored: true, attempts_remaining: 2 }
          : { id: 'response-1' }
    ...
    await user.click(screen.getByRole('radio', { name: 'Move the Patch' }))
    await user.type(screen.getByRole('textbox', { name: 'Why did you choose this repair direction?' }), 'Moving it tests position.')
    ...
    expect(screen.getByText('Did the result support, weaken, or change your original thinking?')).toBeInTheDocument()
    await user.click(screen.getByRole('radio', { name: 'It supported my explanation.' }))
    await user.type(screen.getByRole('textbox', { name: 'Please explain how the result affected your thinking.' }), 'The restored label supports the position repair idea.')
    await user.click(screen.getByRole('button', { name: 'Continue to the Pixel case' }))
    ...
    expect(screen.getByRole('radio', { name: 'Reduce the Pixel attack strength' })).toBeInTheDocument()
    expect(fetchMock.mock.calls.map(([, request]) => request?.body && JSON.parse(String(request.body)).question_key)).toContain('transfer_patch_attempt_1_thinking_effect')
    expect(fetchMock.mock.calls.map(([, request]) => request?.body && JSON.parse(String(request.body)).question_key)).toContain('transfer_patch_attempt_1_thinking_explanation')

    await user.click(screen.getByRole('radio', { name: 'Reduce the Pixel attack strength' }))
    await user.type(screen.getByRole('textbox', { name: 'Why did you choose this repair direction?' }), 'Reducing the strength tests a controlled Pixel repair.')
    await user.click(screen.getByRole('button', { name: 'Continue to Manipulate' }))
    await user.click(await screen.findByRole('button', { name: 'Continue to Reclassify' }))
    await user.click(screen.getByRole('button', { name: 'Reclassify image' }))
    await user.click(await screen.findByRole('button', { name: 'Continue to Compare' }))
    await user.click(screen.getByRole('radio', { name: 'It changed my explanation.' }))
    await user.type(screen.getByRole('textbox', { name: 'Please explain how the result affected your thinking.' }), 'The Pixel result changed how I interpreted the original error.')
